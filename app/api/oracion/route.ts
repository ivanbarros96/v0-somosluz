...
export async function POST(req: NextRequest) {
  ...
  //
  // Se llamaba `telefono`. Al pedir el equipo un teléfono de contacto real
  // (reunión 24/08/2026) hubo que renombrarlo: con el nombre anterior, cada
  // persona que escribiera su número habría visto "enviado" mientras su
  // petición se descartaba acá en silencio.
  if (typeof sitio_web === 'string' && sitio_web.length > 0) {
    return NextResponse.json({ ok: true });
  }
